...
import { useGetBlockQuery, useGetBlockRecordQuery } from '@two/api-react';
...
export default function Block() {
  ...
  const poolReward = mojoToTwo(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToTwo(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const twoFees =
    blockRecord.fees !== undefined ? mojoToTwo(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: twoFees ? `${twoFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Two blockchain
            </Trans>
          </Back>
        }
        action={
