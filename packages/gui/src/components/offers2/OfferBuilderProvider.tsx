...
import type { CalculateRoyaltiesRequest, NFTInfo } from '@two/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@two/api-react';
import { catToMojo, greenBTCToMojo } from '@two/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXTWO = useWatch({
    name: 'requested.xtwo',
  });

  const offeredXTWO = useWatch({
    name: 'offered.xtwo',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXTWO ?? [])
      ...
      .map(({ amount }) =>
        fungibleAssetFromAssetIdAndAmount('xtwo', greenBTCToMojo(amount)),
      ),
    ...(requestedTokens ?? [])
