...
import { type OfferSummaryRecord } from '@two/api';
import {
  Flex,
  FormatLargeNumber,
  StateColor,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xtwo', 'txtwo'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToTwo(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xtwo', 'txtwo'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToTwo(takerEntries[0][1])
