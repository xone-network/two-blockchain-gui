...
import { OfferSummaryRecord, OfferTradeRecord } from '@two/api';
import {
  Box,
  Checkbox,
  Chip,
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: xtwoFee } = methods.getValues();

    const fee = cancelWithTransaction ? greenBTCToMojo(xtwoFee) : new BigNumber(0);
...
function OfferList(props: OfferListProps) {
  ...
  const testnet = useCurrencyCode() === 'TXTWO';
                  ...
                  <Flex flexDirection="column" gap={4} height="100%">
                    <Flex flexDirection="column" gap={2} flexGrow={1}>
                      ...
                      <Typography variant="body1" color="textSecondary">
                        <Trans>
                          Create a file that you can use to trade XTWO, Two
