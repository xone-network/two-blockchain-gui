...
import { OfferTradeRecord } from '@two/api';
import {
  Button,
  Checkbox,
  Dialog,
...
const log = debug('two-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.twochain.top';
...
async function postToOfferBin(
  ...
  if (testnet) {
    return 'https://www.twochain.top/offers';
  }
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. XTWO/RandoCoin
...
async function postToSpacescan(
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txtwo' : 'xtwo'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'txtwo' : 'xtwo'}/offer/${id}`;
}
...
const KeybaseTeamName = 'two_offers';
