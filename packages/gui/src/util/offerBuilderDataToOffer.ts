...
import { greenBTCToMojo, catToMojo } from '@two/core';
...
export default async function offerBuilderDataToOffer(
  ...
  const hasOffer =
    !!offeredXtwo.length || !!offeredTokens.length || !!offeredNfts.length;
  const hasRequest =
    !!requestedXtwo.length || !!requestedTokens.length || !!requestedNfts.length;
  ...
  await Promise.all(
    offeredXtwo.map(async (xtwo) => {
      const { amount } = xtwo;
      if (!amount || amount === '0') {
        throw new Error(t`Please enter an XTWO amount`);
      }
      ...
      if (!hasEnoughBalance) {
        throw new Error(t`Amount exceeds XTWO spendable balance`);
      }
    }),
  );
