...
const getTwoVersion = () => {
  let version = null;
  const exePath = getExecutablePath('two');
  // first see if we can get a two exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or two is on the path
    try {
      version = child_process
