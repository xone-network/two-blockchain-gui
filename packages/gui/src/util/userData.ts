...
export function getUserDataDir(): string {
  const twoRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(twoRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const twoRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${twoRootUserDataPath}`);
  app.setPath('userData', twoRootUserDataPath);
}
