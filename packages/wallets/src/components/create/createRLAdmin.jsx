...
import { AlertDialog, greenBTCToMojo } from '@two/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      twoper_input.value === '' ||
      Number(twoper_input.value) === 0 ||
      !Number(twoper_input.value) ||
      isNaN(Number(twoper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const twoper = greenBTCToMojo(twoper_input.value);
    const twoper_value = Number.parseInt(Number(twoper));
