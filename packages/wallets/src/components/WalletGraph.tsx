...
import { TransactionType, WalletType } from '@two/api';
import type { Transaction } from '@two/api';
import { mojoToTwo, mojoToCAT, blockHeightToTimestamp } from '@two/core';
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip:
        walletType === WalletType.CAT
          ? mojoToCAT(start)
          : mojoToTwo(start).toString(), // bignumber is not supported by react
    });
  });
