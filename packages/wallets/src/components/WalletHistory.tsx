...
import type { Row } from '@two/core';
import { WalletType, TransactionType, toBech32m } from '@two/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToTwo(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
