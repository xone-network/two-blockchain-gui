...
import { useSpendCATMutation, useFarmBlockMutation } from '@two/api-react';
import { SyncingStatus, toBech32m } from '@two/api';
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(
        t`Cannot send two to coloured address. Please enter a two address.`
      );
    }

    if (address.includes('two_addr') || address.includes('colour_desc')) {
      throw new Error(
    ...
    if (address.slice(0, 12) === 'two_addr://') {
